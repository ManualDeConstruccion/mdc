...
class BudgetSubProjectInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['description', 'amount', 'currency', 'start_date', 'is_active', 'budget_land_area',
...
class StageInline(admin.TabularInline):
    model = Stage
    extra = 0
    fields = ['name', 'description', 'due_date', 'payment_trigger', 'is_payment_required', 'payment_amount',
              'payment_type', 'time_frame', 'milestone']
    show_change_link = True
...
class StageAdmin(admin.ModelAdmin):
    list_display = ['name', 'budget_project', 'budget_sub_project', 'due_date', 'payment_trigger',
                    'is_payment_required', 'payment_amount']
